...
import { paymentTypeOptions } from '@/lib/options'
...
/**
 * Polished create/edit modal for Cash Book records.
...
export const CashBookModal = ({
  ...
  open: boolean
  mode: 'create' | 'edit'
  initialRecord: CashBookRecord | null
  saving: boolean
  error: string
  onClose: () => void
  onSubmit: (values: CashBookFormValues) => Promise<void>
}) => {
  const titleId = useId()
  const [values, setValues] = useState<CashBookFormValues>(emptyCashBookFormValues())

  useEffect(() => {
    if (!open) return
    setValues(
      mode === 'edit' && initialRecord
        ? cashBookRecordToFormValues(initialRecord)
        : emptyCashBookFormValues(),
    )
   ...
   */
  const updateField = <K extends keyof CashBookFormValues>(
    key: K,
    value: CashBookFormValues[K],
  ) => {
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h2 id={titleId} className="mt-1 font-[family-name:var(--font-righteous)] text-xl">
                {mode === 'create' ? 'Add Cash Book Data' : 'Edit Cash Book Data'}
              </h2>
            </div>
            <button
              type="button"
            ...
            <Field label="Payment Type">
              <select
                ...
                onChange={(event) =>
                  updateField(
                    'payment_type',
                    event.target.value as CashBookFormValues['payment_type'],
                  )
                }
                className={inputClassName}
              >
...
/**
 * Labeled field wrapper used inside the Cash Book modal.
