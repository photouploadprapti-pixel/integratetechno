...
/**
 * Live Sales Commission list with create/edit modal and role-based ownership.
...
export const SalesCommissionPanel = () => {
  ...
  const [editingRecord, setEditingRecord] = useState<SalesCommissionRecord | null>(null)
  ...
  const columns = useMemo(
    () =>
      isSuperAdmin
        ? salesCommissionColumns
        : salesCommissionColumns.filter((column) => column.key !== 'createdBy'),
    [isSuperAdmin],
  )

  /**
   * Loads sales commission rows visible to the signed-in user.
   */
  const loadRecords = useCallback(async () => {
    ...
    setRecords((data || []) as SalesCommissionRecord[])
    setLoading(false)
  }, [])
   ...
   */
  const openEdit = (record: SalesCommissionRecord) => {
    setModalMode('edit')
  ...
  /**
   * Saves a new or updated sales commission record to Supabase.
   * @param values - Form values from the modal
   */
  const handleSubmit = async (values: SalesCommissionFormValues) => {
    setSaving(true)
    setError('')
    ...
    const payload = {
      lc_number: values.lc_number.trim() || null,
      ...
      lc_amount: parseSalesCommissionNumber(values.lc_amount),
      commission_amount: parseSalesCommissionNumber(values.commission_amount),
    ...
    if (modalMode === 'edit' && editingRecord) {
      ...
      const { error: insertError } = await supabase.from('sales_commission').insert({
        ...payload,
        created_by: user.id,
      })
      if (insertError) {
  ...
  /**
   * Deletes a sales commission record after confirmation.
   * @param record - Record to delete
   */
  const handleDelete = async (record: SalesCommissionRecord) => {
    const confirmed = window.confirm(
      `Delete sales commission for LC "${record.lc_number || 'this entry'}"?`,
    )
    if (!confirmed) return
  ...
  return (
    <>
      <section className="rounded-[24px] border border-[#e4e9f4] bg-white p-4 shadow-[0_10px_30px_rgba(16,36,94,0.06)] sm:p-6">
        <div className="mb-5">
          <label className="sr-only" htmlFor="sales-commission-search">
            Search sales commission records
          </label>
          <input
            id="sales-commission-search"
            type="search"
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            placeholder="Search Sales Commission"
            ...
            <tbody>
              {loading ? (
                <tr>
                  ...
                  >
                    Loading sales commission records...
                  </td>
                </tr>
                ...
                <tr>
                  <td
                    ...
                    className="px-4 py-10 text-center text-[#7a8699]"
                  >
                    No sales commission records yet. Click Add Data to create one.
                  </td>
                </tr>
              ) : (
                filteredRecords.map((record) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5 text-[#1f2a44]">
                      {formatDisplayDate(record.shipment_date)}
                    </td>
                    <td className="whitespace-nowrap px-4 py-3.5 text-[#1f2a44]">
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5 text-[#1f2a44]">
                      {formatSalesCommissionMoney(record.lc_amount)}
                    </td>
                    <td className="whitespace-nowrap px-4 py-3.5 text-[#1f2a44]">
                      {formatSalesCommissionMoney(record.commission_amount)}
                    </td>
