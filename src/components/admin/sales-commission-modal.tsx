...
import {
  emptySalesCommissionFormValues,
  salesCommissionToFormValues,
...
/**
 * Create/edit modal for Sales Commission records.
 ...
 */
export const SalesCommissionModal = ({
  open,
  mode,
  initialRecord,
  saving,
  ...
  open: boolean
  mode: 'create' | 'edit'
  initialRecord: SalesCommissionRecord | null
  saving: boolean
  error: string
  onClose: () => void
  onSubmit: (values: SalesCommissionFormValues) => Promise<void>
}) => {
  const titleId = useId()
  const [values, setValues] = useState<SalesCommissionFormValues>(
    emptySalesCommissionFormValues(),
  )
  ...
  useEffect(() => {
    ...
    setValues(
      mode === 'edit' && initialRecord
        ? salesCommissionToFormValues(initialRecord)
        : emptySalesCommissionFormValues(),
    )
   ...
   */
  const updateField = <K extends keyof SalesCommissionFormValues>(
    key: K,
    value: SalesCommissionFormValues[K],
  ) => {
      ...
      >
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h2 id={titleId} className="mt-1 font-[family-name:var(--font-righteous)] text-xl">
                {mode === 'create' ? 'Add Sales Commission' : 'Edit Sales Commission'}
              </h2>
            </div>
            <button
              type="button"
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            <Field label="LC Number" className="sm:col-span-2">
              <input
                required
                value={values.lc_number}
                onChange={(event) => updateField('lc_number', event.target.value)}
            ...
            <Field label="Shipment Date">
              <input
                ...
                onChange={(event) => updateField('shipment_date', event.target.value)}
                className={inputClassName}
              />
            </Field>
            ...
            <Field label="LC Amount (USD)">
              <input
                ...
                onChange={(event) => updateField('lc_amount', event.target.value)}
                placeholder="0.00"
                className={inputClassName}
              />
            </Field>
...
/**
 * Labeled field wrapper used inside the Sales Commission modal.
